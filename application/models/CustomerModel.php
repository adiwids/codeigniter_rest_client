...
class CustomerModel extends CI_Model
{
	...
	function find_by_nik($nik)
	{
		...
		if(!is_null($result)) {
			return $this->convert_result_to_model($result);
		} else {
			return NULL;
		}
	}
	...
	{
		$nik = $attributes['nik_pelanggan'];
		unset($attributes['nik_pelanggan']);

		$this->db->set('nama_pelanggan', $attributes['nama_pelanggan'])
						 ->set('telepon_pelanggan', $attributes['telepon_pelanggan'])
						 ->where('nik_pelanggan', $nik)
						 ->update(self::TABLE_NAME);

		return $this->find_by_nik($nik);
	}

	function delete($nik)
	{
		if($this->is_nik_taken($nik)) {
			return $this->db->where('nik_pelanggan', $nik)->delete(self::TABLE_NAME);
		} else {
			return FALSE;
		}
	}
	...
	{
		$collection = [];
		$results = $this->db->get(self::TABLE_NAME)->result();
		foreach($results as $result) {
			$instance = $this->convert_result_to_model($result);
